...
## Current State
- **Phase:** **The Dash is built and live system-health is flowing off real hardware.** The Pi serves Trident at `trident.local` (Caddy static export) with Signal K + the **`signalk-rpi-monitor`** plugin running. **AIS** (collision watch), **Dash** (Systems / Power / Weather / Boat with anchor watch), and **Settings** are all real and modular. **Live Pi telemetry** (CPU temp / load / RAM / disk) is wired end-to-end and **verified on the boat box** under `?source=live`. AIS/nav data is still **simulated** until the Pi is in range of the Vesper — but the live Signal K path is now proven for real (the telemetry rides it), so live AIS is the same `?source=live` swap. Power/Weather stay honestly gated until the Cerbo / NGX-1 land; a `?demo=1` flag fills the whole Dash with badged synthetic data for design work. `lib/` has vitest coverage (**117 tests**).
...
- **Shipped this session (session 9 — GPIO horn output layer + live-data default fix):**
...
6. **GPIO horn alarm layer — software DONE & bench-proven (session 9).** `hornState` (pure) + `daemon/relay.ts` (active-low BCM26 via `pinctrl`, safe-HIGH on claim) drive the relay on CPA danger + anchor-drag, edge-triggered in the daemon loop. Remaining: order the **12V horn** + wire it to CH1 (BCM26) at boat install. HAT is mounted; relay is bench-verified active-low + boot-safe.
