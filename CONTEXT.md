...
**Owned by `HARDWARE.md` now** — the physical stack, enclosure, cooling, and HAT mounting live there (session 5 updated them: enclosure changed to the **KKSB Tall** case, the **Active Cooler stays on**, exact HAT is the **Seengreat 3-CH**). See HARDWARE.md → "The Stack" and "Settled Decisions". *(The old session-4 stack — Argon NEO 5 base + Relay HAT deferred — is superseded.)*
...
2. ~~Self-host fonts~~ — **done (session 5):** IBM Plex vendored as latin woff2 in `public/fonts/` + `@font-face` in `globals.css`, Google `@import` dropped. Offline-safe at sea, zero build-time network.
...
## Session Log
- **2026-06-11 (session 5):** **Self-hosted the fonts** — vendored IBM Plex Mono/Sans (latin woff2, 9 weights) into `public/fonts/` and swapped the Google Fonts `@import` for `@font-face` in `globals.css`. Offline-safe at sea with zero build-time network (no `next/font/google` fetch); CSS re-validated through PostCSS. **Settled the physical build and created `HARDWARE.md`** as its source of truth (pushed mid-session by request, commit `736f814`). Hardware decisions banked there: exact HAT is the **Seengreat 3-CH** (HF3FF/005-1ZS relays, 15.5mm tall; pinout CH1/CH2/CH3 = GPIO **26/19/13**, opto-isolated — *not* the Waveshare 26/20/21); the **official Active Cooler stays ON** (corrected a mid-session wrong turn — the NEO 5's fan + heatsink are in its removable top, gone the moment you open it for the HAT); enclosure changed to the **KKSB Tall Aluminum Enclosure for Dual HATs** (on order) because the Seengreat relays put the stacked board ~37mm tall — dual-HAT height the standard KKSB / NEO 5 can't close over; the HAT **stacks on the GPIO** via the KKSB-supplied tall stackable header + 18/20mm spacers (solves the too-long-standoff bench snag), not jumper-wired or panel-mounted. **Argon NEO 5 retired** for this build (kept as a spare). Added project-instruction updates (read HARDWARE.md at session start; its Settled Decisions are binding) — Garry pasting those into the project. Process note: the fonts deploy block wasn't run mid-session (only the HARDWARE.md one was), caught at session end via `git ls-remote` — fonts + this CONTEXT update ship together in the session-end commit.
