...
export default function AnchorReadout({ status, radiusM, setAt, breachSec, self, telemetry, now }) {
  ...
  const sub = noFix
    ...
    : dragging
      ...
      : `at anchor ${elapsed} \u00b7 alarm at ${radiusM} m`;
  ...
  return (
    <>
      ...
      <Hero label="From anchor" value={noFix ? "\u2014" : round(status.distanceM)} unit="m"
        color={dragging ? C.dangerBr : C.value} alarm={dragging} />

      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 9 }}>
        {dragging ? (
          <>
            <Tile label="Past the ring" value={round(-status.roomM)} unit="m" color={C.dangerBr} />
            <Tile label="Moving" value={round(self.cog)} unit="&deg; T" />
            <Tile label="Rate" value={self.sog.toFixed(1)} unit="kt" />
            <Gated label="Depth" hw="NGX-1" value={telemetry.depthM} unit="m" />
          </>
        ) : (
          <>
            <Tile label="Room left" value={noFix ? "\u2014" : round(status.roomM)} unit="m"
              color={level === "caution" ? C.cautionBr : C.safeBr} />
            <Tile label="Bearing to anchor" value={noFix ? "\u2014" : round(status.bearingToSetDeg)} unit="&deg; T" />
            <Gated label="Depth" hw="NGX-1" value={telemetry.depthM} unit="m" />
...
function Hero({ label, value, unit, color, alarm }) {
  return (
    <div style={{
      ...
      <div style={{ fontFamily: FONT_MONO, fontWeight: 700, fontSize: 58, lineHeight: 1, color, marginTop: 6 }}>
...
function Tile({ label, value, unit, color = C.value }) {
  return (
    <div style={tile}>
      <div style={lab}>{label}</div>
      <div style={{ fontFamily: FONT_MONO, fontWeight: 700, fontSize: 24, lineHeight: 1, color, marginTop: 6 }}>
        {value}<span style={{ fontSize: 12, color: C.label, fontWeight: 500 }} dangerouslySetInnerHTML={{ __html: ` ${unit}` }} />
      </div>
    </div>
  );
}
